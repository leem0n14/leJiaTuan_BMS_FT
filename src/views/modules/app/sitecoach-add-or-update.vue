  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/app/sitecoach/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.dataForm.siteId = data.siteCoach.siteId
              this.dataForm.coachId = data.siteCoach.coachId
            }
          })
        }
      })
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/app/sitecoach/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',

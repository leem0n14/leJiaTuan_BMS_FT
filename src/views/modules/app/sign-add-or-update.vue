<template>
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      <!-- <el-form-item label="一级目录" prop="parentId1">
        
        <el-select v-model="dataForm.parentId1" clearable placeholder="请选择一级目录"
          @change="getSignParent2(dataForm.parentId1)">
          <el-option v-for="item in signParentList1" :key="item.id" :label="item.text" :value="item.id">
          </el-option>
        </el-select>
      </el-form-item> -->
      <el-form-item label="二级目录" prop="parentId">
        <!-- <el-input v-model="dataForm.parentId" placeholder="父id"></el-input> -->
        <el-select v-model="dataForm.parentId" clearable placeholder="请选择二级目录">
          <el-option v-for="item in signParentList2" :key="item.id" :label="item.text" :value="item.id">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="封面照片" prop="photo">
        <!-- <el-input v-model="dataForm.photo" placeholder="封面照片"></el-input> -->
      ...
      <el-form-item label="顺序数" prop="number">
        <!-- <el-input v-model="dataForm.number" placeholder="顺序数"></el-input> -->
        <el-input-number v-model="dataForm.number" :min="1" label="顺序数"></el-input-number>
      </el-form-item>
    </el-form>
...
export default {
  data() {
    return {
      ...
      dataRule: {
        parentId: [
          { required: true, message: '二级目录不能为空', trigger: 'blur' }
        ],
        // parentId1: [
        ...
        number: [
          { required: true, message: '顺序数不能为空', trigger: 'blur' }
        ]
      },
      signParentList1: [],
      signParentList2: [],
  ...
  methods: {
    ...
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/sys/sign/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.dataForm.parentId = data.sign.parentId
              this.dataForm.photo = data.sign.photo
              this.dataForm.text = data.sign.text
              this.dataForm.number = data.sign.number
            }
          })
        }
      })
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/sys/sign/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
